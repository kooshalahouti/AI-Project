...
class NaiveBayesClassifier:
    ...
    def calculate_likelihood(self, word, label):
        alpha = 1.0
        numerator = self.class_word_counts[label].get(word, 0) + alpha
        denominator = sum(
            self.class_word_counts[label].values()) + (alpha * len(self.vocab))
        return (numerator / denominator)

    def classify(self, features):
        log_prior = self.calculate_prior()
        class_scores = {label: log_prior.get(
            label, 0.0) for label in self.classes}
        for label in self.classes:
            for word in features:
